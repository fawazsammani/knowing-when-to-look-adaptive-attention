...
#         self.spatial_features = nn.Linear(512, hidden_size)
#         self.global_features = nn.Linear(512, embed_size)
    ...
    def forward(self,images):
        ...
        enc_image = encoded_image.view(batch_size,num_pixels,features) #(batch_size,num_pixels,features)-->(batch_size,64,2048)
        #Get the spatial representation
        spatial_image = F.relu(self.spatial_features(enc_image))       #(batch_size,num_pixels,hidden_size)
        ...
        global_image = F.relu(self.global_features(global_f))          #(batch_size,embed_size)
...
class SpatialAttention(nn.Module):
    def __init__(self, hidden_size, att_dim):
        ...
        # We will set the attention dimension to the same number of pixels, i.e. 64
        self.cnn_att = nn.Linear(hidden_size, att_dim, bias=False)
        self.dec_att = nn.Linear(hidden_size, att_dim, bias=False)
        self.zt = nn.Linear(att_dim,1, bias=False)
    ...
    def init_weights(self):
        ...
        init.xavier_uniform_(self.zt.weight)
    
    def forward(self,spatial_image, decoder_out):
        """
...
class AdaptiveAttention(nn.Module):
    def __init__(self,hidden_size, att_dim):
        ...
        self.dec_out = nn.Linear(hidden_size,att_dim)
        self.sen_out = nn.Linear(hidden_size,att_dim)
        self.att_out = nn.Linear(att_dim,1)
        self.softmax = nn.Softmax(dim=1)  
        self.dropout = nn.Dropout(0.5)
        self.init_weights()
    
    def init_weights(self):
        init.xavier_uniform_(self.dec_out.weight)
        init.xavier_uniform_(self.sen_out.weight)
        init.xavier_uniform_(self.att_out.weight)
    
    def forward(self,decoder_hidden, st, zt, ct):
        """
        decoder_hidden: hidden state of the decoder, of shape: (batch_size, hidden_size)
        st: visual sentinal returned by the Sentinal class, of shape: (batch_size, hidden_size)
        zt: output retuend by the SpatialAttention class, of shape: (batch_size, num_pixels)
        ct: context vector retuend by the SpatialAttention class, of shape: (batch_size, hidden_size)
        """
        out = F.tanh(self.dec_out(self.dropout(decoder_hidden)) + self.sen_out(self.dropout(st)))     # (batch_size,att_dim)
        ...
        return c_hat, beta_t, alpha_hat

class DecoderWithAttention(nn.Module):
    def __init__(self,hidden_size, vocab_size, att_dim, embed_size):
        super(DecoderWithAttention,self).__init__()
        self.fc = nn.Linear(hidden_size, vocab_size)
        self.spatial_attention = SpatialAttention(hidden_size, att_dim)
...
#         self.init_c = nn.Linear(512, hidden_size)  # linear layer to find initial cell state of LSTMCell
    ...
    def forward(self, spatial_image, global_image, encoded_captions, caption_lengths, enc_image):
        ...
        for timestep in range(max(decode_lengths)):
            ...
            # Distinguish between the initial states and the passes states. We need to do this because we have an additional
            # output of the LSTM, which is the sentinal. Otherwise, we will pass the output h to the initial sentinal h. 
            if begining_lstm:
            ...
            if begining_sen:
                ...
                st = self.sentinal(current_input, h[:batch_size_t], c[:batch_size_t])
 
            # Run the spatial attention model
            alpha_t, ct, zt = self.spatial_attention(spatial_image[:batch_size_t],h)
            # Run the adaptive attention model
            c_hat, beta_t, alpha_hat = self.adaptive_attention(h, st, zt, ct)
