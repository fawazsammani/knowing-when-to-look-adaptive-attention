...
batch_size = 40                         # set to 20 when finetuning the encoder
...
checkpoint = 'BEST_checkpoint_10.pth.tar'    # path to checkpoint, None at the begining - BEST NOW IS 0.239
...
def beam_search_eval(encoder, decoder, img, beam_size, wrong):
    ...
    # Encode
    image = img.cuda().unsqueeze(0)  # (1, 3, 254, 254)
...
def evaluate(encoder, decoder):
    ...
    for iteration, (imgs, caps, caplens, allcaps) in enumerate (val_loader):
        #imgs    --> shape (batch_size, 3, 254, 254)
